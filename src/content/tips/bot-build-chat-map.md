...
| **Grok Bot** | クラウド上の自分用コンピュータを持つ同僚。チャットの前置き保存ではない | Grok Bot アプリ / Cursor | 仕事を任せておきたい人 |
...
FAQ は、Grok Bot を grok.com のチャットや公式アプリとは別物だと書いています。クラウド上のコンピュータを持ち、アプリにログインして仕事を進める同僚です。デスクトップアプリ（macOS / Windows / Linux）は <a href="https://x.ai/bot" rel="external">x.ai/bot</a> から入れます。チャットに前置きを保存する機能ではありません。

作るときに書くとよいのは、名前、主たる仕事、進め方です。指示が長いときの型は次です。
...
対象プランは公式内で食い違っています。2026-08-26 のニュースは SuperGrok と Cursor Pro を含む一方、取得時点の導入ページは SuperGrok Plus / Heavy と Cursor Pro+ 以上だけです。Cursor の個人向け Pro は、料金表に月 $20 と Grok Bot access とあります。**画面で確認してください。** Bot の利用量は Grok や Cursor の上限とは別に数えます。grok.com のチャットや Imagine の上限は付きません（<a href="/tips/free-vs-supergrok/">払う場所</a>）。
