...
同じアカウントの Bot は、その 1 台を共有します。grok のログインも共有です。Anthropic の Claude Code を、同じコンピュータで Bot に使わせる手順は、<a href="/tips/bot-uses-claude-code/">Claude Code は、Bot のコンピュータで動かす</a>に書きました。
...
- 「重い仕事は grok に渡せばいい」: grok.com のチャットではありません。Grok Build です。打つコマンドが grok です
- 「画面の中の Build と同じ」: 別です。画面の中は grok.com と公式アプリで、2026-08-19 の発表では全プランです
- 「チャットに入れれば残る」: 会話が終われば消えます。入れる場所は Bot のコンピュータです
- 「自分の PC に入れたから Bot でも使える」: コンピュータが違います。Bot 側にも入れます
- 「出てきたから公開してよい」: まだです。Bot が読んでから使います
- 「更新しても grok は残る」: 消えることがあります。残したいものは `/workspace` です
