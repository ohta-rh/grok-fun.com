...
同じ Grok ですが、**画面と入れる機能が入口ごとに違います。** 「アプリにはあるのに Web に無い」「X からだと出ない」は珍しくありません。目当ての機能が見つからないときは、別の入口を試すのが最短です。
...
どの入口でも、最初の 1 行で「日本語で」と範囲を指定すると安定します（<a href="/tips/japanese-fix/">TIPS 01</a>）。
